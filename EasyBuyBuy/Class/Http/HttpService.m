...
{
    [self post:[self mergeURL:advertisement_list] withParams:params completionBlock:^(id obj) {
        if (obj) {
            NSString * statusStr = [NSString stringWithFormat:@"%@",obj[@"status"]];
            if ([statusStr isEqualToString:@"1"]) {
...
-(void)fetchSysNotificationWithParams:(NSDictionary *)params completionBlock:(void (^)(id))success failureBlock:(void (^)(NSError *, NSString *))failure
{
    [self post:[self mergeURL:user_push_list] withParams:params completionBlock:^(id obj) {
        if (obj) {
            NSString * statusStr = [NSString stringWithFormat:@"%@",obj[@"status"]];
            if ([statusStr isEqualToString:@"1"]) {
                ...
                NSArray * responseObject = obj[@"result"];
                if ([responseObject count]) {
                    NSArray * tempArray = [self mapModelProcess:responseObject withClass:[NotiObj class]];
                    success(tempArray);
                }else
                {
                    success(nil);
...
-(void)fetchProductNotificationWithParams:(NSDictionary *)params completionBlock:(void (^)(id))success failureBlock:(void (^)(NSError *, NSString *))failure
{
    [self post:[self mergeURL:user_push_list] withParams:params completionBlock:^(id obj) {
        if (obj) {
            NSString * statusStr = [NSString stringWithFormat:@"%@",obj[@"status"]];
...
-(void)getPublishListDataWithParams:(NSDictionary *)params completionBlock:(void (^)(id))success failureBlock:(void (^)(NSError *, NSString *))failure
{
    [self post:[self mergeURL:my_publish_list] withParams:params completionBlock:^(id obj)
     {
         if (obj) {
             NSString * statusStr = [NSString stringWithFormat:@"%@",obj[@"status"]];
             if ([statusStr isEqualToString:@"1"]) {
                 ...
                 if ([responseObject count]) {
                     ...
                     NSArray * tempArray = [self mapModelProcess:responseObject withClass:[PublicListData class]];
                     success(tempArray);
                 }else
                 {
                     success(nil);
